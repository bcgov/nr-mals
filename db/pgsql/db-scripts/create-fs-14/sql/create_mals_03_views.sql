...
create or replace view mal_print_dairy_farm_notification_vw as
	with base as (   
		select rslt.licence_id,
			...
			to_char(current_date, 'fmMonth, yyyy') levy_month_year,
			site.site_details,
...
create or replace view mal_dairy_farm_test_infraction_vw as
	...
	result1 as (
		...
		select rslt.id test_result_id
		    ...
		    --  SPC1
		    ,case 
		       when spc1_day is not null
		       then cast(concat(test_year,'-',test_month,'-',spc1_day) as date)
		       else null
		     end spc1_date
		    ...
		    --  SCC
		    ,case 
		       when scc_day is not null
		       then cast(concat(test_year,'-',test_month,'-',scc_day) as date)
		       else null
		     end scc_date
		    ...
		    --  CRY
		    ,case 
		       when cry_day is not null
		       then cast(concat(test_year,'-',test_month,'-',cry_day) as date)
		       else null
		     end cry_date
		    ...
		    --  FFA
		    ,case 
		       when ffa_day is not null
		       then cast(concat(test_year,'-',test_month,'-',ffa_day) as date)
		       else null
		     end ffa_date
		    ...
		    --  IH
		    ,case 
		       when ih_day is not null
		       then cast(concat(test_year,'-',test_month,'-',ih_day) as date)
		       else null
		     end ih_date
