...
if [[ (! -z "$APP_DB_OWNER_USERNAME") &&  (! -z "$APP_DB_OWNER_PASSWORD") && (! -z "$APP_DATABASE")]]; then
  ...
  psql "$1" -w -c "CREATE DATABASE ${APP_DATABASE} OWNER ${APP_DB_OWNER_USERNAME} ENCODING '${APP_DB_ENCODING:-UTF8}' LC_COLLATE = '${APP_DB_LC_COLLATE:-en_US.UTF-8}' LC_CTYPE = '${APP_DB_LC_CTYPE:-en_US.UTF-8}'"
