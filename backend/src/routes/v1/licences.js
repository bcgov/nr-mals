    ...
    payloads.map(async (payload) => {
      ...
      const count = await prisma.mal_licence_parent_child_xref.count({
        where: {
          ...
          child_licence_id: payload.mal_licence_mal_licenceTomal_licence_parent_child_xref_child_licence_id.connect.id,
        },
      });
...
async function findDairyTestResultHistory(params, skip, take) {
  ...
  result.sort(function (a, b) {
    return (
      (a.spc1_date === null) - (b.spc1_date === null) || -(a.spc1_date > b.spc1_date) || +(a.spc1_date < b.spc1_date)
    );
  });
...
router.put("/dairyactions/:licenceId(\\d+)", async (req, res, next) => {
  ...
  if (
    ...
    const previousResults = fetchResults.map((record) => dairyTestResult.convertToLogicalModel(record));
    ...
    const filteredResults = previousResults.filter((x) => x.spc1Date >= elevenMonths);

    let infractionCount = 0;

    if (req.body.thresholdId === constants.DAIRY_TEST_THRESHOLD_IDS.IBC) {
      infractionCount = Math.min(filteredResults.filter((x) => x.spc1CorrespondenceDescription !== null).length, 3);
    } else if (req.body.thresholdId === constants.DAIRY_TEST_THRESHOLD_IDS.SCC) {
      infractionCount = Math.min(filteredResults.filter((x) => x.sccCorrespondenceDescription !== null).length, 3);
    } else if (req.body.thresholdId === constants.DAIRY_TEST_THRESHOLD_IDS.CRY) {
      infractionCount = Math.min(filteredResults.filter((x) => x.cryCorrespondenceDescription !== null).length, 3);
    } else if (req.body.thresholdId === constants.DAIRY_TEST_THRESHOLD_IDS.IH) {
      infractionCount = Math.min(filteredResults.filter((x) => x.ihCorrespondenceDescription !== null).length, 3);
    }

    const infraction = infractions.find((x) => x.previous_infractions_count === infractionCount);
...
router.put("/renew/:licenceId(\\d+)", async (req, res, next) => {
  ...
  await findLicence(licenceId)
    .then(async (record) => {
      ...
      const update = licence.convertToLogicalModel(record);
      ...
      if (update.bondContinuationExpiryDate !== null) {
        update.bondContinuationExpiryDate = new Date(update.bondContinuationExpiryDate);
      }
...
router.put("/:licenceId(\\d+)/registrants", async (req, res, next) => {
  ...
  const registrantsToCreate = registrants.filter((r) => r.status === REGISTRANT_STATUS.NEW);
  const registrantsToDelete = registrants.filter((r) => r.status === REGISTRANT_STATUS.DELETED);
  const registrantsToUpdate = registrants.filter((r) => r.status === REGISTRANT_STATUS.EXISTING);
...
router.post("/", async (req, res, next) => {
  ...
  const newRegistrants = req.body.registrants
    ? req.body.registrants.filter((r) => r && r.status === REGISTRANT_STATUS.NEW)
    : [];

  req.body.companyName = newRegistrants[0] !== undefined ? newRegistrants[0].companyName : undefined;

  const licencePayload = licence.convertToPhysicalModel(populateAuditColumnsCreate(req.body, now, now), false);
