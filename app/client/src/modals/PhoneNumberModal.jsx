...
export default function PhoneNumberModal(
    { phone, existingTypes, closeModal, submit }
){
    const onSubmit = (data) => {
        ...
        if (data.number === undefined || !data.number.match(/^$|\(\d{3}\) \d{3}-\d{4}/g)) {
    ...
    const phoneTypes = [ { value: PHONE_NUMBER_TYPES.PRIMARY, description:"Primary Phone" },
                           { value: PHONE_NUMBER_TYPES.SECONDARY, description:"Secondary Phone" },
                           { value: PHONE_NUMBER_TYPES.FAX, description:"Fax" },
    ];

    const phoneOptions = phoneTypes.filter(x => !existingTypes.includes(x.value) || x.value === phone.phoneNumberType);
    ...
    <Form onSubmit={handleSubmit(onSubmit)} noValidate>
        ...
        <Modal.Header closeButton>
            <Modal.Title>{phone.number  ? "Edit a Phone Number" : "Add a Phone Number"}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
            <Form.Row>
                <Col lg={6}>
                    <Form.Group controlId="phoneNumberType">
                        ...
                        <Form.Control
                            ...
                            ref={register}
                            defaultValue={phone.phoneNumberType ?? PHONE_NUMBER_TYPES.PRIMARY}
                            custom
                        >
                            {phoneOptions.map((x) => (
