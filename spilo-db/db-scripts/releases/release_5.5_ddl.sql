...
CREATE VIEW mals_app.mal_licence_comment_vw
...
CREATE PROCEDURE mals_app.pr_generate_print_json_licence_comments(
	...
	with licence_comments as (
		select 
			...
			json_agg(json_build_object('LicenceNumber',               lic.licence_number,
				                       ...
				                       'CommentDate',                 lic.comment_date,
				                       'Comment',                 	  lic.comment)
				                       order by lic.comment_date) licence_json,
			count(*) num_rows
...
ALTER PROCEDURE mals_app.pr_generate_print_json_licence_comments(in int4, inout int4) OWNER TO mals;
GRANT ALL ON PROCEDURE mals_app.pr_generate_print_json_licence_comments(in int4, inout int4) TO mals;
GRANT EXECUTE ON PROCEDURE mals_app.pr_generate_print_json_licence_comments(in int4, inout int4) TO mals_app_role;
